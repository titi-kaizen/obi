...
export default function GenerateWeeklyBriefButton() {
  ...
  async function generate() {
    ...
    try {
      ...
      const res = await fetch('/api/generate-weekly-brief', {
        method: 'POST',
      ...
      const range = data.weekRange ? ` (${data.weekRange.from} → ${data.weekRange.to})` : ''
      setMessage(`Brief semanal generado — ${data.articlesAnalyzed} artículos${range}`)
      setState('done')
